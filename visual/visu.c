...
int		loop_hook(t_win *win)
{
	...
	{
		ft_bzero(win->addr, win->dat.height * win->dat.net *
				win->dat.width * win->dat.net * 4);
		mlx_put_image_to_window(win->mlx_ptr, win->win_ptr,
				win->img_ptr, win->dat.loc_x, win->dat.loc_y);
		if (win->keys.right)
		{
...
void	fig_size(t_win *win)
{
	...
	{
		win->dat.net = 40;//58
		win->dat.loc_y = (H / 2) - ((win->dat.height * 40) / 2);
		win->dat.loc_x = (W / 2) - ((win->dat.width * 40) / 2);
	}
	if (win->dat.height == 24)
	{
		win->dat.net = 20; //25
		win->dat.loc_y = (H / 2) - ((win->dat.height * 20) / 2);
		win->dat.loc_x = (W / 2) - ((win->dat.width * 20) / 2);
	}
	if (win->dat.height == 100)
	{
		win->dat.net = 7; //10
		win->dat.loc_y = (H / 2) - ((win->dat.height * 7) / 2);
		win->dat.loc_x = (W / 2) - ((win->dat.width * 7) / 2);
	}
}
